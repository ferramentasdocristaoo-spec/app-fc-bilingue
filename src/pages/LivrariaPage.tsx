...
import { LIBRARY_PRODUCT_SLUG, LIBRARY_VOLUME_COUNT, libraryProduct } from "@/data/library";
      ...
      <article className="max-w-3xl overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <div className="grid gap-6 p-5 sm:grid-cols-[150px_1fr]">
          ...
          <div className="flex flex-col justify-center">
            <span className="mb-2 w-fit rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">{t("livraria.badge", { count: LIBRARY_VOLUME_COUNT })}</span>
            <h2 className="font-display text-2xl font-bold">{product.title}</h2>
            <p className="mt-2 text-sm text-muted-foreground">{t("livraria.productDescription")}</p>
